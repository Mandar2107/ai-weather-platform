...
const sendSmsWithTwilio = async (settings, phone, message) => {
  const { twilioAccountSid, twilioAuthToken, twilioPhoneNumber } = getTwilioConfig(settings);

  if (!twilioAccountSid || !twilioAuthToken || !twilioPhoneNumber) {
    throw new Error("Twilio SMS settings are incomplete");
  }

  console.log("[TWILIO SMS] Sending message");
  console.log("[TWILIO SMS] From:", twilioPhoneNumber);
  ...
  console.log("[TWILIO SMS] Message accepted");
  console.log("[TWILIO SMS] SID:", data.sid);
  console.log("[TWILIO SMS] Status:", data.status);
...
exports.sendSMS = async (phone, message) => {
  ...
  if (settings.smsProvider === "twilio") {
    return await sendSmsWithTwilio(settings, phone, message);
  }
